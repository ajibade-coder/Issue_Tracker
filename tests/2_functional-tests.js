...
suite('Functional Tests', function() {
    ...
    suite("3 Get request Tests", function () {
      ...
      test("View issues on a project with one filter: GET request to /api/issues/{project}", function (done) {
        chai
          ...
          .get("/api/issues/peter")
          .query({
            _id: "670d2337c4ceee57607e2ea9",
          })
          .end(function (err, res) {
            assert.equal(res.status, 200);
            assert.deepEqual(res.body[0], {
              _id: "670d2337c4ceee57607e2ea9",
              issue_title: "failed engine",
              issue_text: "leaking chambers",
              created_on: "2024-10-14T13:57:11.000Z",
              updated_on: "2024-10-14T13:57:11.000Z",
              created_by: "engineer",
              assigned_to: "rex",
              open: true,
              status_text: "pending",
    ...
    suite("5 Put request Tests", function () {
      test("Update one field on an issue: PUT request to /api/issues/test-data-put", function (done) {
        chai
          .request(server)
          .put("/api/issues/peter")
          .send({
            _id: "670d2397c4ceee57607e2eb1",
            issue_title: "updated engine",
          })
          .end(function (err, res) {
            assert.equal(res.status, 200);
            assert.equal(res.body.result, "successfully updated");
            assert.equal(res.body._id, "670d2397c4ceee57607e2eb1");
      ...
      test("Update multiple fields on an issue: PUT request to /api/issues/{project}", function (done) {
        chai
          ...
          .end(function (err, res) {
            assert.equal(res.status, 200);
            assert.equal(res.body.result, "successfully updated");
            assert.equal(res.body._id, "670d2397c4ceee57607e2eaf");
